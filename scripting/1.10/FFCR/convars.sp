...
SetupConVars()
{
	...
	g_hCVar_ExcludeChatTriggers = CreateConVar("ffcr_exclude_chat_triggers", EXCLUDE_CHAT_TRIGGERS, "Excludes (1) or includes (0) SourceMod chat triggers in the chat flood detection", _, true, 0.0, true, 1.0);
	g_hCVar_MuteVoiceLoopback = CreateConVar("ffcr_mute_voice_loopback", MUTE_VOICE_LOOPBACK, "Mute players enabling voice_loopback (1) or allow its use (0)", _, true, 0.0, true, 1.0);
	//- Chat -//
	g_hCVar_ChatInterval = CreateConVar("ffcr_chat_interval", FLOOD_CHAT_INTERVAL, "Minimum average interval in seconds between a players chat- and radio-messages (0 to disable)", _, true, 0.0, true, 20.0);
	...
	//- Hard Flood -//
	g_hCVar_HardInterval = CreateConVar("ffcr_hard_interval", FLOOD_HARD_INTERVAL, "Time in seconds in which <ffcr_hard_num> commands are allowed (0 to disable)", _, true, 0.0, true, 20.0);
	...
	g_hCVar_HardBanTime = CreateConVar("ffcr_hard_ban_time", FLOOD_HARD_BAN_TIME, "Number of minutes a client is banned for when hard-flooding (0 = Forever)", _, true, 1.0, true, 20160.0);
	//- Namecheck -//
	g_hCVar_NameInterval = CreateConVar("ffcr_name_interval", FLOOD_NAME_INTERVAL, "Time in seconds in which <ffcr_name_num> name changes are allowed (0 to disable)", _, true, 0.0, true, 600.0);
	...
	g_hCVar_NameBanTime = CreateConVar("ffcr_name_ban_time", FLOOD_NAME_BAN_TIME, "Number of minutes a client is banned for when name-flooding (0 = Forever)", _, true, 1.0, true, 20160.0);
	//- Connect Check -//
	g_hCVar_ConnectInterval = CreateConVar("ffcr_connect_interval", FLOOD_CONNECT_INTERVAL, "Time in seconds in which <ffcr_connect_num> connects are allowed (0 to disable)", _, true, 0.0, true, 60.0);
	...
	g_hCVar_ConnectBanTime = CreateConVar("ffcr_connect_ban_time", FLOOD_CONNECT_BAN_TIME, "Number of seconds a client is IP-banned for when connect-flooding (0 = Forever)", _, true, 5.0, true, 600.0);
